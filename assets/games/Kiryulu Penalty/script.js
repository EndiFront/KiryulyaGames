...
function initSDK() {
  ...
  if (typeof YaGames !== "undefined") {
    YaGames.init()
      ...
      .then((p) => {
        ysdkPlayer = p;
        return ysdkPlayer.getData(["gameState"]);
      })
      .then((data) => {
      ...
      .catch((err) => {
        console.warn("YSDK Init Warn:", err);
        loadGameState();
        updateStartButtonUI();
        notifyGameReady();
      });
...
function showWinModal() {
  if (ysdkInstance?.features?.GameplayAPI)
...
function draw() {
  ...
  const playerHeight = mobile ? 70 : 96;
  const enemyHeight = mobile ? 65 : 88;
  ...
  enemies.forEach((e) => {
    ...
    ctx.beginPath();
    ctx.ellipse(e.x, e.y + (mobile ? 26 : 36), mobile ? 14 : 18, 5, 0, 0, Math.PI * 2);
    ctx.fill();
    ...
    if (e.maxHp > 1) {
      const barW = mobile ? 28 : 36;
      const barH = 5;
      const barX = e.x - barW / 2;
      const barY = e.y - (mobile ? 38 : 50);
  ...
  if (!isExitingLevel) {
    ...
    ctx.beginPath();
    ctx.ellipse(player.x, player.y + (mobile ? 28 : 40), mobile ? 16 : 22, 6, 0, 0, Math.PI * 2);
    ctx.fill();
...
function saveGameState() {
  ...
  if (ysdkPlayer) {
    ysdkPlayer.setData({ gameState }, true).catch((err) => {
      console.error("Ошибка сохранения в YSDK:", err);
    });
  }
}
...
// --- БЕЗОПАСНАЯ ОБРАБОТКА РЕКЛАМЫ ---
let lastFullscreenAdTime = 0;
const AD_COOLDOWN_MS = 120000;

function showSafetyFullscreenAdv(onComplete) {
  const now = Date.now();

  if (
    !ysdkInstance ||
    !ysdkInstance.adv ||
    now - lastFullscreenAdTime < AD_COOLDOWN_MS
  ) {
    onComplete();
    return;
  }
...
document.getElementById("startGameBtn")?.addEventListener("click", () => {
  ...
  showSafetyFullscreenAdv(launchGame);
});
...
document.getElementById("nextLevelBtn")?.addEventListener("click", () => {
  ...
  showSafetyFullscreenAdv(proceedToNextLevel);
});
  ...
  if (ysdkInstance?.adv) {
    ysdkInstance.adv.showRewardedVideo({
      callbacks: {
        ...
        onError: (e) => {
          console.error("Ad Revive Error:", e);
          SoundEngine.isMuted = false;
          reviveAction();
        },
      },
  ...
  if (ysdkInstance?.adv) {
    ysdkInstance.adv.showRewardedVideo({
      callbacks: {
        ...
        onError: (e) => {
          console.error("Reward Ad Error (+300 coins):", e);
          SoundEngine.isMuted = false;
        },
      },
    });
...
function showTrainingHintIfNeeded() {
  ...
  if (isMobileDevice() && tabletHintEl) {
